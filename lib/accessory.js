...
Accessory.prototype.get = async function(callback, context, displayName, iid) {
  ...
  if (this.reachable) {
    ...
    this.log.debug("Accessory.get [iid %s] '%s' '%s' '%s' '%s' '%s'", iid, this.name, service_name, this.service_types[this.name], c, value);
    await get(this.name, service_name, this.service_types[this.name], c, value, callback);
