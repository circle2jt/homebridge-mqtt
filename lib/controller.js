...
var api, HapAccessory, Service, Characteristic, UUIDGen, Categories;
...
Controller.prototype.addAccessory = async function(api_accessory) {
  ...
  var category = api_accessory.category
  var service_name;
  var ack, message;
  ...
  if (typeof name === "undefined") {
    ...
    var newAccessory
    if (category) {
      newAccessory = new HapAccessory(name, uuid, Categories[category]);
    } else {
      newAccessory = new HapAccessory(name, uuid);
    }
    ...
    await this.Model.add(name, service_name)
...
Controller.prototype.addService = async function(api_accessory) {
...
Controller.prototype.removeAccessory = async function(name) {
...
Controller.prototype.removeService = async function(api_accessory) {
  ...
  if (typeof(this.accessories[name]) === "undefined") {
  ...
  } else if (typeof this.hap_accessories[name].getService(service_name) === "undefined") {
    ...
    await this.Model.remove(name, service_name)
...
Controller.prototype.updateReachability = async function(accessory) {
  ...
  if (typeof name === "undefined") {
    ...
    await Promise.all([
      this.Model.updateReachability(name, reachable),
      this.setValue({
        name, 
        service_name: accessory.name,
        characteristic: 'Reachable',
        value: accessory.reachable,
      }),
    ]) 
...
Controller.prototype.getCharacteristic = function(api_accessory) {
  ...
  } else if (typeof this.hap_accessories[name].getService(service_name, service_name) === "undefined") {
    ack = false; message = "accessory '" + name + "', service_name '" + service_name + "' not found.";
...
Controller.prototype.get = async function (name, service_name, service_type, c, value, callback) {

  return await this.Model.get(name, service_name, service_type, c, value, callback);
}

Controller.prototype.set = async function (name, service_name, service_type, c, value, callback) {

  await this.Model.set(name, service_name, service_type, c, value, callback);
}

Controller.prototype.setValue = async function (api_accessory) {
  ...
  if (!result.isValid) {
    ...
    await this.Model.save({
      name: api_accessory.name, 
      service_name: api_accessory.service_name,
      characteristic: api_accessory.characteristic, 
      value: result.value
    })
    if (!result.isValid) {
...
Controller.prototype.identify = async function (name) {
  ...
  await this.Model.identify(name, manufacturer, model, serialnumber, firmwarerevision);
}
