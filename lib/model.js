...
Model.prototype.start = function() {
  ...
  this.log("Connecting..");
  client = new Redis(this.config.redis);
...
clientSub.on('connect', function () {
  ...
  clientSub.on('pmessage', function (_, topic, payload) {
  ...
  clientSub.on('error', function (error) {
    this.log.error("on.error %s", error);
  }.bind(this));
  
  clientSub.on('reconnect', function () {
    this.log.warn("on.reconnect <to analyze>");
  }.bind(this));


  clientSub.on('offline', function () {
...
Model.prototype.add = function(name, service_name) {
  ...
  return client.hset(`device/${id}`, "id", id)
}
...
Model.prototype.remove = function(name, service_name) {
  ...
  if (isRemoveAll) {
    return client.keys(`device/${name}.*`).then(keys => {
      if (!keys.length) {
        return
      }
  ...
  return client.del(`device/${name}.${service_name || name}`)
}
