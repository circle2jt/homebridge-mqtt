...
var set_timeout, client, clientSub, clientDB, getterDevices;
...
Model.prototype.start = function() {
  ...
  options.clientId = this.config.client_id || 'homebridge-redis_' + Math.random().toString(16).substr(2, 8);
  this.log("clientId = %s", options.clientId);
  
  this.log("Connecting..");
  client = new Redis(this.config.redisPubSub);
  clientSub = new Redis(this.config.redisPubSub);
  if (this.config.redisPubSub === this.config.redisDB) {
    clientDB = client
  } else {
    clientDB = new Redis(this.config.redisDB)
  }
  // todo client.end();
...
Model.prototype.add = function(name, service_name) {
  const id = `${name}.${service_name || name}`
  return clientDB.hset(`device/${id}`, "id", id)
}

Model.prototype.save = function(msg) {
  return Promise.all([
    clientDB.hset(`device/${msg.name}.${msg.service_name || msg.name}`, msg.characteristic, msg.value),
    client.publish('device/updated', JSON.stringify(msg))
  ])
}

Model.prototype.remove = function(name, service_name) {
  const isRemoveAll = !service_name || name === service_name
  if (isRemoveAll) {
    return clientDB.keys(`device/${name}.*`).then(keys => {
      if (!keys.length) {
        return
      }
      return clientDB.del(...keys)
    })
  }
  return clientDB.del(`device/${name}.${service_name || name}`)
}

Model.prototype.get = function(name, service_name, service_type, c, value, callback) {
  if (!getterDevices.has(`${name}.${service_name || name}`)) {
    return
  }
