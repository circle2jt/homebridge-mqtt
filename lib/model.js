...
var set_timeout, client, redis, getterDevices;
...
Model.prototype.start = function() {
  ...
  this.publish_options_low = {
    retain: false,
    qos: 0
  }
  this.publish_options_high = {    
    retain: false,
    qos: 2 
  }
  ...
  redis = new Redis(this.config.redis || process.env.REDIS_URI)
...
Model.prototype.add = function(name, service_name) {
  ...
  return redis.hset(`device/${id}`, "id", id)
}

Model.prototype.save = function(msg) {
  return redis.multi()
    .hset(`device/${msg.name}.${msg.service_name || msg.name}`, msg.characteristic, msg.value)
...
Model.prototype.remove = function(name, service_name) {
  ...
  if (isRemoveAll) {
    return redis.keys(`device/${name}.*`).then(keys => {
      if (!keys.length) {
        return
      }
  ...
  return redis.del(`device/${name}.${service_name || name}`)
}
...
Model.prototype.get = function(name, service_name, service_type, c, value, callback) {
  ...
  client.publish(topic, JSON.stringify(msg), this.publish_options_low);
...
Model.prototype.set = function(name, service_name, service_type, c, value, callback) {
  ...
  return Promise.all([
    this.save(),
    client.publish(topic, JSON.stringify(msg), this.publish_options_high),
  ]).then(() => callback())
