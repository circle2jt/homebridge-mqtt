...
var client, clientSub, getterDevices, deviceUpdatedCharacteristic;
...
Model.prototype.start = async function() {
  ...
  if (this.config.deviceUpdatedCharacteristic && this.config.deviceUpdatedCharacteristic !== '*') {
    deviceUpdatedCharacteristic = (this.config.deviceUpdatedCharacteristic || '')
      .split(',')
      .map(e => e.trim())
      .filter(e => e)
  }
...
Model.prototype.save = async function (msg) {
  ...
  const pipe = client.multi().hset(id, data)
  if (!deviceUpdatedCharacteristic || deviceUpdatedCharacteristic.includes(msg.characteristic)) {
    pipe.publish('device/updated', JSON.stringify(msg))
  }
...
Model.prototype.set = async function (name, service_name, service_type, c, value, callback) {
  ...
  // await Promise.all([
  //   // this.save(msg), // TODO: Maybe ignore it
